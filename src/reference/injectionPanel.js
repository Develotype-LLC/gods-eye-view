...
export function mountInjectionPanel({viewer, dataManager, layer}) {
  ...
  panel.innerHTML = `<header><strong>DISPOSAL & INJECTION</strong><button data-hide aria-label="Hide disposal wells">✕</button></header><small data-freshness>Connecting to Texas RRC…</small><label>Reporting month<input type="month" data-month></label><p data-total></p><small>● Teal: volume &nbsp; ● Cream: source zero<br>● Gray: no record for selected month</small><label>Find a well<select data-well><option value="">Select on map or choose an API-8…</option></select></label><div data-details></div><p class="ref-note">Records are delayed. Missing months are not zero. Status and locations are historical. Spatial overlap does not establish causation.</p>`;
  ...
  function sync() {
    ...
    panel.querySelector('[data-freshness]').textContent = data.connection?.warning || (data.sourceRetrievedAt ? `Texas RRC · refreshed ${new Date(data.sourceRetrievedAt).toLocaleString()} · cached up to 6 hours · records through ${data.period[1]}` : 'HeavenWatch historical archive · original retrieval date unavailable');
    ...
    detail.append(make('h3', `API-8 ${well.api8}`), make('p', `UIC: ${well.uics.join(', ')} · ${well.permitType === 1 ? 'W-14' : 'H-1'} disposal`),
      make('p', `Source flags: ${well.plugged ? 'plugged' : 'no plugged flag'}; ${well.canceled ? 'canceled' : 'no canceled flag'}. These do not prove current operation.`),
    ...
    if (well.history.length) {
      detail.append(make('p', `${well.history.length} available months · ${well.history[0].month} → ${well.history.at(-1).month}`));
    ...
    } else detail.append(make('p', 'No monthly history in this dataset.'));
    detail.append(make('small', `Join ID ${well.id} is synthetic; completion suffix 0000 is not verified. ${data.sourceMode === 'rrc-api' ? 'Trailing source-zero months omitted; pressure zero treated as missing.' : 'Archived zero volumes may include original blanks converted upstream.'}`));
  }
